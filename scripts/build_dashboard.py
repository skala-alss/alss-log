...
def patch_member_columns_in_block(block_md, pid_to_states, participants):
    ...
    try:
        folder_col = _find_col_idx(
            header, exact=["폴더"], fuzzy=["폴더", "folder", "📁"]
        )
    ...
    for i in range(data_start, len(lines)):
        ...
        for idx, seat in enumerate(seats):
            ...
            if do_write:
                if DEBUG:
                    print(
                        f"[debug] patch: pid={pid} seat={seat} state={state} -> '{sym}' (was '{cells[col]}')"
                    )
                cells[col] = sym
            else:
                if DEBUG:
                    print(
                        f"[debug] patch: pid={pid} seat={seat} state={state} (no change)"
                    )
...
def _replace_block_exact(
    text: str, marker: str, new_md_inside: str, s: int, e: int
) -> str:
    return (
        text[:s]
        + f"<!--START:{marker}-->\n\n{new_md_inside}\n\n<!--END:{marker}-->"
...
def render_week_readme_members_only(week_cfg, participants, states_by_group):
    ...
    for g in week_cfg.get("groups", []):
        ...
        new_block = patch_member_columns_in_block(
            block.strip("\n"), states_by_group[g["key"]], participants
        )
...
def _member_owns_path(path: str, pid: int, member: dict) -> bool:
    ...
    if re.search(rf"/boj_{pid_s}[^/]*/", p):
        for k in klist:
            if (
                bn_norm.startswith(k + "_")
                or bn_norm.startswith(k + "-")
                or k in _split_tokens(base_noext)
            ):
                return True
    # 느슨: 토큰 공존
...
def _parse_pid_and_seat_from_basename(
    base_noext: str, participants, assigned_universe: Set[int]
...
def _pid_from_path(
    path: str, participants, assigned_universe: Set[int]
) -> Optional[int]:
    """
    경로에서 배정 PID 판별:
      1) 경로/디렉터리의 boj_#### 우선
    ...
    pid2, _seat_guess = _parse_pid_and_seat_from_basename(
        base_noext, participants, assigned_universe
    )
    if pid2:
        return pid2
...
def _run_bytes(cmd: str, cwd: str = ROOT_DIR) -> bytes:
    p = subprocess.Popen(
        cmd, shell=True, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL
    )
    out, _ = p.communicate()
    return out or b""
...
def list_all_refs() -> List[str]:
    ...
    try:
        ...
        refs = [
            ln.strip().strip("'") for ln in (out1 + out2).splitlines() if ln.strip()
        ]
    except Exception:
    ...
    if DEBUG:
        print(
            f"[debug] refs detected: {len(order)} -> {', '.join(order[:12])}{' ...' if len(order)>12 else ''}"
        )
    return order
...
def collect_submit_commits_on_main(
        ...
        if DEBUG:
            if not paths:
                try:
                    ...
                    mflag = False
                print(
                    f"[debug] submit-commit has NO files (merge={mflag}) sha={sha} subj={subj_norm}"
                )
            else:
...
def collect_repo_files_all(
    weeks_cfg, refs: List[str], participants
) -> Dict[int, List[str]]:
    """
...
def classify_states_repo(
    week_cfg, members, problems: List[int], repo_index_all: Dict[int, List[str]]
    ...
    for m in members:
        ...
        if DEBUG:
            print(
                f"[debug] {handle} week={week_cfg['id']} solved={len(solved)} / assigned={len(pset)}"
            )
        for pid in problems:
            if pid not in solved:
                ...
                continue
            owned = any(
                _member_owns_path(path, pid, m) for path in repo_index_all.get(pid, [])
            )
            if DEBUG and pid in solved:
                cand = repo_index_all.get(pid, [])
                print(
                    f"[debug] DURING-check seat={seat} handle={handle} pid={pid} cand={len(cand)} -> {'DURING' if owned else 'PRE'}"
                )
                if cand and not owned:
                    for p in cand[:6]:
                        ok = _member_owns_path(p, pid, m)
                        print(
                            f"         try: member={m.get('file_key') or m.get('name') or m.get('github')} vs {p} => {ok}"
                        )
...
def build_submission_attribution(
    weeks_cfg, participants, states_bundle
    ...
    if DEBUG:
        print(
            "[debug][trend] DURING_by_seat_sizes:",
    ...
    if DEBUG:
        dbg = {
            s: {wk: len(ps) for wk, ps in sorted(wmap.items())}
            for s, wmap in cand_by_seat_week.items()
        }
    ...
    # 커밋에서 실제로 등장한 주차 라벨을 정렬해 사용 (weeks.yaml 밖 라벨도 포함)
    commit_wk_labels = sorted(
        {wk for seat_map in cand_by_seat_week.values() for wk in seat_map.keys()},
        key=lambda s: (0, int(s)) if s.isdigit() else (1, s),
    )

    for wk in commit_wk_labels:  # 커밋에서 관측된 주차 순서대로
    ...
    # 브랜치 diff 보정 (weeks.yaml 밖 라벨도 허용)
    if CHECK_BRANCHES:
        refs = list_all_refs()
        problems_root = infer_problems_root(weeks_cfg)

        # refs에서 weekNN-<alias> 패턴을 실제로 수집
        branch_hits = []  # (wk_lab, seat, ref)
        if DEBUG and not branch_hits:
            print("[debug] branch-match: no weekNN-* refs detected")
        for r in refs:
            base = r.split("/")[-1]
            m = re.match(r"^week\s*0*(\d{1,3})-([A-Za-z0-9_\-]+)$", base, re.IGNORECASE)
            if not m:
                continue
            wk_lab = f"{int(m.group(1)):02d}"
            alias_key = _norm_token(m.group(2))
            seat = seat_by_branch_key.get(alias_key)
            if seat:
                branch_hits.append((wk_lab, seat, r))

        # 수집된 브랜치 라벨들 기준으로 보정 진행
        for wk_lab, seat, r in branch_hits:
            diff_paths = list_diff_paths_vs_main(r, problems_root)
            if DEBUG:
                print(
                    f"[debug] branch-match: ref={r} wk={wk_lab} diff_paths={len(diff_paths)}"
                )
            for p in diff_paths:
                ...
                submission_map[seat][pid] = wk_lab
                seen_by_seat[seat].add(pid)
    ...
    if ALSS_TREND_FALLBACK_DURING:
        ...
        for w in weeks_cfg:
            ...
            ws = set(
                pid for g in (w.get("groups") or []) for pid in g.get("problems", [])
            )
...
def build_dashboard_payload(participants, weeks_cfg, states_bundle):
    ...
    labels_from_submission = {
        wk for seat_map in submission_map.values() for wk in seat_map.values()
    }
...
def main():
    ...
    states_bundle = (
        {}
    )  # week_id -> group_key -> {pid -> {seat -> 'PRE'|'DURING'|'NONE'}}
    for w in weeks_cfg:
        w_id = w["id"]
        states_bundle[w_id] = {}
        for g in w.get("groups") or []:
            states = classify_states_repo(
                w, participants, g["problems"], repo_index_all
            )
    ...
    out_path = (
        os.path.join(ROOT_DIR, EXPORT_JSON_PATH)
        if not os.path.isabs(EXPORT_JSON_PATH)
        else EXPORT_JSON_PATH
    )
